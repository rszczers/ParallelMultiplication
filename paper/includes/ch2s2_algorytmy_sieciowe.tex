...
\end{align*}
\noindent o ile \(B_{13}\), \(B_{23}\) i \(B_{33}\) znajdowałyby się na węźle odpowiednio dla kroków \(t=1, 2, 3\). Uwzględniając obliczenia na węzłach \(P_{12}\) i \(P_{13}\) możemy zainicjalizować sieć jak na rys. \ref{}
...
Widać, że jeśli macierz \(B\) jest wstępnie rozmieszczona z zastosowaniem jednego przesunięcia, to po zakończeniu obliczeń w węzłąch \(P_{11}\), \(P_{12}\), \(P_{13}\) otrzymujemy pierwszy wiersz macierzy \(C\).
...
Jeśli wstępnie przesuniemy drugi wiersz o jedną kolumnę w lewo i trzeci wiersz -- o dwie, wówczas możemy przeprowadzić odpowiednie dodawania i mnożenia występujące w wyrażeniu \eqref{eq:cannon_per_node} dla każdego węzła sieci w każdym kroku algorytmu. 

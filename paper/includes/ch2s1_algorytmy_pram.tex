...
\State \(t[i,j,k] \gets a[i,k]*b[k,j]\)\Comment{Obliczanie składowych iloczynów skalarnych}
\EndParFor
...
\noindent Zgodnie z def. \ref{def:cost-optimal} algorytm \emph{nie} jest optymalny względem kosztu.
...
\EndParFor
\end{algorithmic}
\caption{Algorytm mnożenia macierzy dla \(n\) procesorów.\cite{Czech}}
\label{alg:pram_multiply_2}
\end{algorithm}


% W algorytmie \ref{alg:pram_multiply_1} między \(n^3\) węzłów zostało rozdzielone 
